...
7. [プロファイル](#プロファイル)
8. [モデル一覧（GPT-5.4 世代）](#モデル一覧gpt-54-世代)
9. [推論レベル](#推論レベル)
...
model_verbosity = "medium"              # low | medium | high (GPT-5.4 default: medium)
...
| モデル | 特徴 | 認証 |
|--------|------|------|
| `gpt-5.4` | デフォルト。汎用・高推論力 | ChatGPT / API |
| `gpt-5.4-mini` | 2倍高速。Web検索・軽量タスクに最適 | ChatGPT / API |
| `gpt-5.4-nano` | 最速・最安。分類・抽出・ランキング向け | API キーのみ |

`gpt-5.4-nano` は `codex login`（ChatGPT アカウント）では使用不可。
`OPENAI_API_KEY` 環境変数での認証が必要。

## 推論レベル

`model_reasoning_effort` に指定可能な値:
`none`(GPT-5.2+) / `minimal` / `low` / `medium`(デフォルト) / `high` / `xhigh`

- `none`: 推論トークンを生成しない。最も低レイテンシ。プロンプトで「考えて」と促すと精度向上
- `low`: 軽い推論。Web検索のクエリ生成・結果要約には十分
- `medium`: デフォルト。ほとんどのタスクに適切
- `high` / `xhigh`: 複雑な推論・数学・コード生成向け
